...
| Workspace groups | `workspace-group list/create/ungroup/delete/rename/collapse/expand/pin/unpin/add/remove/set-anchor/new-workspace/set-color/set-icon/move/focus`; `workspace.group.*` | Missing | No Limux workspace group model found. Needs data model, UI state, persistence, CLI/API, and tests. |
...
| Events stream | `events.stream`; CLI `events --after --cursor-file --name --category --reconnect --limit --no-ack --no-heartbeat` | Missing | No durable event bus with sequence cursors, heartbeats, category/name filters, or reconnect protocol found. |
...
| Config/settings/themes/docs | `settings`, `config doctor/check/validate/path/paths/docs/reload/get/set`, `themes list/set/clear`, `docs settings/shortcuts/api/browser/agents` | Partial | Limux has app config, shortcut docs, and theme behavior. Remaining: CMUX-compatible CLI surface, config path precedence, config doctor, theme names, and docs commands. |
| CLI globals and environment | `--socket`, `--password`, `--json`, `--id-format`, `--window`; `CMUX_SOCKET_PATH`, `CMUX_SOCKET`, `CMUX_SOCKET_PASSWORD`, `CMUX_WORKSPACE_ID`, `CMUX_SURFACE_ID`, `CMUX_TAB_ID` | Partial | Limux has socket/json/id-format and `LIMUX_*` env. Remaining: CMUX env aliases, password option, window scoping, and no-socket help probes. |
