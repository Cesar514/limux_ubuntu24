...
[] 3. Implement the remaining Codex-only CMUX `codex-teams` production watcher flow: launch private Codex app-server/root TUI, run `__codex-teams-watch`, bridge app-server approval requests through Feed, subscribe to spawned subagent threads, and open managed subagent panes through `surface.split`. \
