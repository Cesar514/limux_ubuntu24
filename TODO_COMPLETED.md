...
[x] 2026-07-02T11:17:05Z Implement and verify CMUX/tmux local split/layout parity in `limux-cli`: `split-window`/`splitw` route to `surface.split` with tmux direction, cwd, target, command, and `-P` output handling, `select-layout` routes to `workspace.equalize_splits` with main-layout orientation mapping, and unsupported detached split focus preservation fails loudly. \
